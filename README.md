...
- **[April 2026]** For some resolvers (e.g., Knot Resolver and Technitium), restarting the container fails to clean up the DNS cache, which could lead to unexpected interference between RUC tests. To resolve this, a new script [renew_resolver.py](poc_scripts/renew_resolver.py) is added to remove the previous resolver container and create a new one for each single test.
